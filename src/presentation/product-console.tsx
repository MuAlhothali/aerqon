...
export function ProductConsole({ model }: { model: PrototypeProjection }) {
  ...
  function navigate(next: View, selected?: string) {
    setView(next); setDetailId(selected);
  }
  ...
  return <div className="app-shell" dir={language === "ar" ? "rtl" : "ltr"}>
    ...
    <aside className="sidebar"><div className="brand"><span className="brand-symbol" aria-hidden="true">A</span><div className="wordmark">AERQON<span>CLOUD</span></div></div><p className="nav-eyebrow">{t("workspace")}</p>
      <nav aria-label={t("workspace")}>{navigation.map((key) => <button key={key} aria-current={view === key ? "page" : undefined} onClick={() => navigate(key)}><ConsoleIcon name={key} />{t(key)}{key === "findings" && <span className="nav-count">{model.details.length}</span>}</button>)}</nav>
      <div className="sidebar-footer"><span className="local-dot" aria-hidden="true" /><strong>{t("company")}</strong><p>{t("offline")}</p><small>v0.1 / SYNTHETIC</small></div>
    </aside>
    <div className="workspace"><header className="topbar"><div className="assessment-context"><span className="workspace-avatar" aria-hidden="true">N</span><div><strong>{t("company")}</strong><span>{t("review")}</span></div></div>
      <div className="preferences"><label><span className="sr-only">Language</span><select aria-label="Language" value={language} onChange={(e) => update(e.target.value, theme)}><option value="en">English</option><option value="ar">العربية</option></select></label>
        <label><span className="sr-only">{t("theme")}</span><select aria-label={t("theme")} value={theme} onChange={(e) => update(language, e.target.value)}>{(["light", "dark", "system"] as const).map((key) => <option key={key} value={key}>{t(key)}</option>)}</select></label></div>
    </header>
    ...
    {view === "assessment" && <AssessmentOverview model={model} language={language} onFinding={openDetail} onNavigate={navigate} activeTable={renderFindingsTable(model.actions.filter((item) => item.finding.remediationState !== "RESOLVED"))} />}
    {view === "findings" && (selected ? <><button className="back-link" onClick={() => navigate("findings")}><span className="direction-arrow" aria-hidden="true">←</span> {t("back")}</button><FindingDetail detail={selected} model={model} language={language} onRecheck={() => navigate("recheck")} /></> : <>
      <p className="page-intro">{t("priorityIntro")}</p><div className="filter-bar"><label className="search-label"><span className="sr-only">{t("search")}</span><input type="search" value={query} onChange={(e) => setQuery(e.target.value)} placeholder={t("search")} /></label>
