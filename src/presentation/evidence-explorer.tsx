...
export function EvidenceExplorer({ projection, language }: { projection: AssessmentProjection; language: Language }) {
    ...
    <div className="evidence-workbench">
      ...
      <section className="evidence-inspector" aria-label={t("inspector")} aria-live="polite">
        {selected ? <>
          ...
          <h3>{t("linked")}</h3><div className="linked-evaluations">{projection.evaluations.filter((item) => item.evidenceIds.includes(selected.evidenceId)).map((item) => <div key={item.evaluationId}><Technical>{item.ruleId} / v{item.ruleVersion}</Technical><Status value={item.evaluationState} language={language} /></div>)}</div>
