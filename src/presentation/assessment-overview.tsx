...
interface Props {
  model: PrototypeProjection; language: Language; activeTable: ReactNode;
  onFinding: (detail: FindingDetailProjection) => void;
  onNavigate: (view: "actions" | "findings" | "recheck" | "coverage") => void;
}

export function AssessmentOverview({ model, language, activeTable, onFinding, onNavigate }: Props) {
  ...
  return <>
    ...
    <div className="scope-strip">
      ...
      <Technical>{model.baseline.assessment.regions.join(" / ")}</Technical>
    </div>
    <div className="metric-row briefing-metrics">
      {([[unresolved, "activeWork", "actions", "actions"], [model.summary.resolved, "resolved", "recheck", "recheck"],
    ...
    <div className="overview-grid">
      <Section title={t("activeWork")} aside={<span className="section-count">{unresolved}</span>}>
        <p className="section-intro">{t("activeWorkNote")}</p>{activeTable}
      </Section>
